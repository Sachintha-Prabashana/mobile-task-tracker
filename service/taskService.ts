...
export const getTaskById = async (id: string) => {
    ...
    if (data.userId !== user.uid) throw new Error('Unauthorized access to task');
...
export const deleteTask = async (id: string) => {
    ...
    if (snap.data().userId !== user.uid) throw new Error('Unauthorized access to task');
    await deleteDoc(ref);
}
...
export const getAllTasksByStatus = async (isComplete: boolean) => {
    const user = auth.currentUser
    if (!user) throw new Error('User not authenticated.')
